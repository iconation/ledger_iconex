...
class App extends Component {
    ...
    checkError = async (callback) => {
        try {
            ...
            const {
                address
            } = await icx.getAddress(path, false, true);
            callback();
        } catch (error) {
    ...
    setSelectedAccount = async (wallet) => {
        const {
            i18n,
            popupType
        } = this.state;
    ...
    handleCopy = (e, index) => {
        ...
        handleCopy(`span.copyKey${index}`, index, copyState, this.setState.bind(this))
    }
    ...
    render() {
        ...
        const popupTypeLabel = popupType === POPUP_TYPE.VOTING ? i18n.button.select : i18n.button.transfer

        return ( <
            div className = "popup-wrap" >
            <
            div className = "popup address wallet"
            style = {
                {
                    // width: 1160,
                    // height: 568,
            ...
            style = {
                {
                    ...
                    tbody > {
                        walletList.map((wallet, index) => ( <
                            ...
                            <
                            p className = {
                                `link ${copyIndex === index ? 'complete' : ''}`
                            }
                            onClick = {
                                (e) => this.handleCopy(e, index)
                            } >
                            <
                            span className = {
                                `ellipsis copyKey${index}`
                            } > {
                                wallet.account
                            } < /span>{copyState === COPY_STATE['on'] ? (<em>{i18n.button.copyFinish}</em > ): ( < em > {
                                i18n.button.copyDepositAddress
                            } < /em>)} <
                            /p> <
                            /td> <
                            td > {
                                `${convertNumberToText(wallet.balance)}`
                            }
                            ICX < /td> <
                            td style = {
                                {
                            ...
                            className = "btn-type-link" > < span > {
                                i18n.button.tracker
                            } < /span></button >
                            <
                            button onClick = {
                                () => this.setSelectedAccount(wallet)
                            }
                            className = "btn-type-exchange" > < span > {
                                popupTypeLabel
                            } < /span></button >
                            <
                            /td> <
                    ...
                    colSpan = "5" > < LoadingComponent type = 'black' / > < /td> <
                    /tr> <
                    /tbody> <
                    /table>
                )
            ...
            return ( <
                ...
                /li> {
                    pageNum.map(btn => {
                        return ( <
                            li className = {
                                `${btn.disabled && 'disabled'} ${page === btn.num && 'selected'}`
                            }
                            key = {
                                btn.num
                            } >
